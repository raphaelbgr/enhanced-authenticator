      ...
      <div className="bg-slate-800 rounded-t-2xl w-full max-w-md max-h-[85vh] overflow-y-auto border-t border-slate-700">
        ...
        <div className="p-5 space-y-6">
                ...
                onClick={async () => {
                  try {
                    if (settings.biometricEnabled) {
                      await window.api.biometricDisable()
                      updateSetting('biometricEnabled', false)
                    } else {
                      await window.api.biometricEnable()
                      updateSetting('biometricEnabled', true)
                    }
                  } catch {
                    // Biometric setup failed, don't update setting
                  }
                }}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-medium text-slate-300">API Access</div>
                <div className="text-xs text-slate-500">
                  {!settings.apiEnabled
                    ? 'API server is disabled'
                    : settings.apiListenAll
                      ? 'API Enabled (network)'
                      : 'API Enabled (localhost)'}
                </div>
              </div>
              <button
                onClick={() => updateSetting('apiEnabled', !settings.apiEnabled)}
                className={`relative w-11 h-6 rounded-full transition-colors ${
                  settings.apiEnabled ? 'bg-accent' : 'bg-slate-600'
                }`}
              >
                <div
                  className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
                    settings.apiEnabled ? 'translate-x-5.5' : 'translate-x-0.5'
                  }`}
                />
              </button>
            </div>
            ...
            {settings.apiEnabled && (
              <>
                <div className="flex items-center justify-between pl-3">
                  <div>
                    <div className="text-sm font-medium text-slate-300">Allow Network Access</div>
                    <div className="text-xs text-slate-500">Exposes API to your local network</div>
                  </div>
                  <button
                    onClick={() => {
                      if (settings.apiListenAll) {
                        updateSetting('apiListenAll', false)
                      } else {
                        setShowNetworkWarning(true)
                      }
                    }}
                    className={`relative w-11 h-6 rounded-full transition-colors ${
                      settings.apiListenAll ? 'bg-accent' : 'bg-slate-600'
                    }`}
                  >
                    <div
                      className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
                        settings.apiListenAll ? 'translate-x-5.5' : 'translate-x-0.5'
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="pl-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (port >= 1024 && port <= 65535) updateSetting('apiPort', port)
                    }}
                    className="w-24 px-2 py-1 rounded bg-surface border border-slate-600 text-xs text-slate-300 font-mono focus:outline-none focus:border-accent"
                  />
                </div>
              </>
            )}
          ...
          {/* Network access warning dialog */}
          {showNetworkWarning && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[60]">
              <div className="bg-slate-800 rounded-xl border border-slate-600 max-w-sm mx-4 p-5 space-y-4">
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-amber-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                      d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
                  </svg>
                  <h3 className="text-sm font-semibold text-amber-400">Security Warning</h3>
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">
                  This will expose your authenticator API to <strong>all devices on your network</strong>.
                  Anyone on the same network could potentially access your TOTP codes.
                </p>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Only enable this on networks you fully trust. For remote access, an <strong>SSH tunnel</strong> with
                  localhost is the recommended approach:
                </p>
                <code className="block text-[10px] text-slate-400 bg-slate-900 rounded px-2 py-1.5 font-mono">
                  ssh -L {settings.apiPort}:127.0.0.1:{settings.apiPort} user@host
                </code>
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={() => setShowNetworkWarning(false)}
                    className="flex-1 py-2 rounded-lg border border-slate-600 text-slate-300 hover:bg-surface-hover text-xs font-medium transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => {
                      updateSetting('apiListenAll', true)
